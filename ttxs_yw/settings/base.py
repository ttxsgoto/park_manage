...
1. 进入项目目录
    - cd park_manage
2. 安装依赖关系(前提需要先安装python3.7和pip)
    - pip install -r deploy/requirements
3. 创建数据库(如果为sqlsite可不用该两步步骤)
...
7. 创建用户3个用户(admin01/admin02/admin03)密码同用户名
    - python manage.py createsuperuser   # 执行3次生成三个用户名
        - admin01 / admin01
        - admin02 / admin02
        - admin-3 / admin03
8. 启动django 服务，端口为8080
    - python manage.py runserver 0.0.0.0:8080


前端vue服务说明:
1. 进入项目目录
    - cd park_vue/park_manage
2. 安装依赖关系(前提需要安装node.js和npm)
    - npm install
3. 解决依赖问题,如果有警告提醒
    - npm audit fix --force
4. 启动vue前端服务
...
其他说明：
    - 如果启动成功,浏览器会自动打开，然后输入用户名密码,即可进入系统
    - 如果不想要现在的数据,直接删除park_manage/ttxs_yw/db.sqlite3文件,
      执行上述步骤(4,5,6,7,8)即可
"""